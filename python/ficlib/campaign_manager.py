...
class CampaignManager:
    ...
    target:             str
    binary:             str
    # Where the campaign writes its artifacts: the per-FIC fault files
    # under `faults/` and the per-thread gvsoc work dirs `work_<tid>/`.
    workdir:            str
    ...
    # The gvsoc executable. Resolved from PATH by default, which
    # `sourceme.sh` points at the SDK install.
    gvsoc_bin:          str = "gvsoc"
    ...
    def __post_init__(self):
        ...
        os.makedirs(f'{self.workdir}/faults', exist_ok=True)
    ...
    def run_gvsoc(self, tid, run_id, fault_opts=None, golden_run=False):
        cmd = [
            ...
            f"--work-dir={self.workdir}/work_{tid}",
    ...
    def do_golden_run(self):
        ...
        for fic in self.fics:
            ...
            ffp = f'{self.workdir}/faults/{san_fic}'
        ...
        for fic in self.fics:
            # Cycle count
            san_fic = self.san_fics[fic]
            ccp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/cycle_count_{san_fic}'
            ...
            mdp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/memories_data_{san_fic}'
            ...
            rdp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/regfiles_data_{san_fic}'
            ...
            pdp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/prefetchers_data_{san_fic}'
            ...
            cdp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/caches_data_{san_fic}'
            ...
            # Hashes (if any)
            if fic in self.fic_to_hashcmds:
                hp = f'{self.workdir}/work_{GOLDEN_RUN_TID}/hashes_{san_fic}'
            ...
            if os.path.exists(f'{self.workdir}/faults/{san_fic}'):
                os.remove(f'{self.workdir}/faults/{san_fic}')

        if os.path.exists(f'{self.workdir}/work_{GOLDEN_RUN_TID}'):
            shutil.rmtree(f'{self.workdir}/work_{GOLDEN_RUN_TID}')
    ...
    def worker(self, tid):
        ...
        while self.runs_remaining[tid] > 0:
            ...
            for fic in all_fic:
                ...
                san_fic = self.san_fics[fic]
                ffp = f"{self.workdir}/faults/{san_fic}_{tid}"
                with open(ffp, 'w') as ff:
                    ff.write('\n'.join(lines))
            ...
            if status == GVSOC_RUN_ERROR or status == GVSOC_RUN_TIMEOUT:
                ...
                for fic in self.fic_to_hashcmds:
                    san_fic = self.san_fics[fic]
                    hp = f"{self.workdir}/work_{tid}/hashes_{san_fic}"
                    with open(hp, 'r') as hf:
                        for line in hf:
                            id_s, h_s = line.split()
                            id = int(id_s)
        ...
        for fic in self.fics:
            san_fic = self.san_fics[fic]
            if os.path.exists(f'{self.workdir}/faults/{san_fic}_{tid}'):
                os.remove(f'{self.workdir}/faults/{san_fic}_{tid}')

        if os.path.exists(f'{self.workdir}/work_{tid}'):
            shutil.rmtree(f'{self.workdir}/work_{tid}')
